...
namespace SettingsManager
{
    ...
    public partial class Setting : ModSetting
    {
        ...
        [SettingsUIDisplayName("SettingsManager.SettingsManager.Mod.Setting.Profile")]
        [SettingsUIValueVersion(typeof(Setting), nameof(GetProfileDropdownItemsVersion))]
        public string ProfileID { set; get; } = SettingsProfile.DefaultID;

        public SettingsProfile Profile => Profiles.Find((p) => p.ID.ToString() == ProfileID);
        ...
        public bool Delete
        {
            set
            {
                var p = Profile;
                Profiles.Remove(p);
                ProfileID = Profiles.Last().ID.ToString();
                OnProfilesChanged();
            }
        }
